...
class SockitText extends FlxTypedSpriteGroup<SockitChar>
{
	static var fonts = new Map<SockitFont, SockitFontData>();
	static var casesAllowed = new Map<SockitFont, Case>();

	public var text(default, set):String = "";

	var font:SockitFontData;
	...
	public function new(x = 0.0, y = 0.0, text:String, fontName:SockitFont = Bold)
	{
		if (!fonts.exists(fontName))
			fonts[fontName] = new SockitFontData(fontName);
		font = fonts[fontName];
	...
	function appendTextCased(text:String)
	{
		...
		for (i in 0...splitValues.length)
		{
			switch (splitValues[i])
			{
				...
				case char:
					{
						var charSprite:SockitChar;
						if (group.members.length <= charCount)
							charSprite = new SockitChar(atlas, char);
						else
						{
...
class SockitChar extends FlxSprite
{
	...
	function set_char(value:String)
	{
		if (this.char != value)
		{
			...
			animation.addByPrefix("anim", prefix + '0', 24);
			animation.play("anim");
			updateHitbox();
		}
...
private class SockitFontData
{
	...
	public function new(name:SockitFont)
	{
		atlas = Paths.loadSparrow("UI/fonts/" + name.getName().toLowerCase());
