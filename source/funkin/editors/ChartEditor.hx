...
import funkin.system.Song.SongData;
import funkin.system.Song;
import funkin.ui.Icon;
...
class ChartEditor extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		var bg = new FlxSprite(0, 0).loadGraphic(FunkinPaths.image('UI/menus/menuDesat'));
		bg.alpha = 0.6;
		...
		leftIcon = new Icon('bf', false);
		rightIcon = new Icon('dad', false);
	...
	function addSongUI():Void
	{
		...
		var characters:Array<String> = FunkinUtil.listFromFolder('assets/characters/', '.json');
		...
		var stages:Array<String> = FunkinUtil.listFromFolder('assets/stages/', '.json');

		var stageDd = new FlxUIDropDownMenu(140, 150, FlxUIDropDownMenu.makeStrIdLabelArray(stages, true), function(stage:String)
		{
			_song.stage = stages[Std.parseInt(stage)];
		});

		stageDd.selectedLabel = _song.stage;
	...
	function loadSong(daSong:String):Void
	{
		...
		FlxG.sound.playMusic(FunkinPaths.inst(daSong), 0.6);

		// WONT WORK FOR TUTORIAL OR TEST SONG!!! REDO LATER
		vocals = new FlxSound().loadEmbedded(FunkinPaths.voices(daSong));
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		if (id == FlxUICheckBox.CLICK_EVENT)
		{
			...
			switch (label)
			{
				case 'Must hit section':
					_song.sections[curSection].mustHitSection = check.checked;
				...
				case "Alt Animation":
					_song.sections[curSection].altAnim = check.checked;
			}
		}
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			{
				_song.sections[curSection].bpm = nums.value;
				updateGrid();
			}
		}
	...
	{
		steps = recalculateSteps();

		Conductor.songPos = FlxG.sound.music.time;
		_song.song = typingShit.text;

		strumLine.y = getYfromStrum((Conductor.songPos - sectionStartTime()) % (Conductor.stepCrochet * _song.sections[curSection].lengthInSteps));
		
		if (FlxG.keys.justPressed.X)
			toggleAltAnimNote();


		if (beats % 4 == 0 && steps >= 16 * (curSection + 1))
		{
			trace(steps);
		...
		curRenderedNotes.forEach(function(note:Note) {
			if (Conductor.songPos > note.strumTime)
				note.alpha = 0.6;
			else
				note.alpha = 1;
		});

		FlxG.watch.addQuick('daBeat', beats);
		FlxG.watch.addQuick('daStep', steps);
		...
		if (FlxG.mouse.justPressed)
		{
			...
			else
			{
				if (FlxG.mouse.x > gridBG.x
					...
					&& FlxG.mouse.y < gridBG.y + (GRID_SIZE * _song.sections[curSection].lengthInSteps))
				{
		...
		bpmTxt.text = bpmTxt.text = "Time: " + Std.string(FlxMath.roundDecimal(Conductor.songPos / 1000, 0))
			+ "\nSection: "
			+ curSection
			+ "\nBeat: "
			+ beats
	...
	function copySection(?sectionNum:Int = 1)
	{
		...
		{
			var strum = note[0] + Conductor.stepCrochet * (_song.sections[daSec].lengthInSteps * sectionNum);
	...
	function updateHeads():Void
	{
		...
		{
			leftIcon.changeIcon(_song.characters[0]);
			rightIcon.changeIcon(_song.characters[1]);
		}
		else
		{
			leftIcon.changeIcon(_song.characters[0]);
	...
	function updateGrid():Void
	{
		...
		{
			Conductor.setBPM(_song.sections[curSection].bpm);
	...
	private function addNote():Void
	{
		...
		_song.sections[curSection].notes.push([noteStrum, noteData, 0, false]);

		curSelectedNote = _song.sections[curSection].notes[_song.sections[curSection].notes.length - 1];

		if (FlxG.keys.pressed.CONTROL)
		{
			_song.sections[curSection].notes.push([noteStrum, (noteData + 4) % 8, 0, false]);
		}
	...
	{
		PlayState.song = Song.songParse(FlxG.save.data.autosave);
		FlxG.resetState();
	}
